...
2. **Software (optional):** Before `iccp` owns the pins, you can drive gate BCM lines **LOW** (e.g. a small oneshot or `ExecStartPre=` in **your** systemd unit). The repo no longer ships a default gate-hold script; use **hardware pull-downs** for defined power-up behavior when the SoC is not driving the line.
