...
- **Code:** [`control.py`](../control.py) `Controller.update()` (path FSM, PWM) and `advance_shift_fsm()` (shift-based `state_v2`: Off, Probing, Polarizing, Protected, Overprotected, Fault).
- **Sensors:** [`sensors.py`](../sensors.py) `read_all_real` / sim.
...
So **Tier 2 =** [`iccp_runtime.py`](../iccp_runtime.py) outer-loop block + `ReferenceElectrode` + `Controller` methods, **not** a separate process. Requirements language for `all_protected` and `T_SYSTEM_STABLE` is in [`docs/iccp-requirements.md`](iccp-requirements.md) §2.2.

## Related

- Commissioning (Phases 1–3, native + ramp): [`commissioning.py`](../commissioning.py)
- Logging / `latest.json`: [`logger.py`](../logger.py)
