...
2. **Physical placement (two goals, one install):**  
   - **Cathode proximity:** Position the **tip near the fin / cathode surface** in the condensate film (on the order of **2–3 mm** is a practical target) so static and slow reads are not swamped by extra electrolyte IR that instant-off cannot remove.  
   - **Current-path hygiene:** Also keep the cell **out of the densest anode feed / return plume** where local drive current would dominate—e.g. toward the **far end of the drain pan** from embedded anodes, consistent with (1). Document the actual wire route for each install.  
   If you only optimize for “far from anodes” but float the tip in **bulk** water away from the fin, you can reintroduce IR error; if you only optimize for “close to fin” but sit in the worst current filament, noise rises. Good field practice satisfies **both** where the geometry allows.
...
**Two-phase open-circuit calibration** (anodes out vs in bath, MOSFETs off): [galvanic-offset-calibration.md](galvanic-offset-calibration.md).
