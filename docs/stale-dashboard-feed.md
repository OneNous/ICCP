...
1. The **controller** (`iccp start`, whether foreground or via the `iccp` systemd unit) is **not running** or crashed, or
...
|--------|-------------------|
| Controller running? | `systemctl status iccp` (adjust unit name if yours differs), or `ps aux \| grep -E 'iccp\|main:app'` |
...
| Dashboard `LOG_DIR` | Same one-liner in the shell **where you start** `iccp dashboard`, or ensure systemd `Environment=COILSHIELD_LOG_DIR=...` matches **exactly** |
