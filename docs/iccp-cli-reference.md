...
- **Default (interactive):** optional **Press Enter** pause on a TTY before Phase 1 open-circuit native (confirm bath + anodes ready, MOSFETs off). Skipped in **`--sim`**, when stdin is not a TTY, with **`--no-anode-prompts`**, or **`COMMISSIONING_ANODE_PLACEMENT_PROMPTS = False`**, or env **`ICCP_COMMISSION_NO_ANODE_PROMPTS=1`**. **Field mode** — **`COMMISSIONING_FIELD_MODE=True`** or **`ICCP_COMMISSION_FIELD_MODE=1`**: skip that pause (automation / headless).
