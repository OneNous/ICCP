...
On first start (no `commissioning.json` in the project root), `main.py` runs **self-commissioning**: **Phase 1** turns all channels off, waits `COMMISSIONING_SETTLE_S`, averages reference INA219 samples → saves **`native_mv`**. **Phase 2** ramps per-channel target current (`COMMISSIONING_RAMP_STEP_MA` per step): after each **`COMMISSIONING_RAMP_SETTLE_S`** regulate segment, PWM goes to **0 %** for **`COMMISSIONING_INSTANT_OFF_S`**, the reference is sampled (**instant-off / IR-free** style), then PWM restores; **shift** = `native_mv − that reading`. When shift reaches `TARGET_SHIFT_MV` **five** consecutive times, **`commissioned_target_ma`** and timestamps are written to `commissioning.json`. Defaults favor **polarization soak** on high-Z bench water; tune faster on real coil + condensate.
