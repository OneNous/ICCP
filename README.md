...
**I2C / INA219 sanity check (especially after wiring or mux changes):** run **`iccp probe`** on the Pi; use **`--init`** if you want to force INA219 CONFIG writes before raw reads. This uses **smbus2** only and is the quickest way to see NACKs, wrong TCA9548A ports, or bus errors separate from the main control loop. If **TCA9548A** is enabled in `config.settings` (`I2C_MUX_ADDRESS` / `I2C_MUX_CHANNEL_*`), **STEP 1** runs a flat “idle” scan (often only the mux at **0x70**), then **STEP 1b** selects each configured downstream port and pings the expected INA/ADS—same idea as your manual per-channel `write_byte(0x70, 1 << ch)` before `i2cdetect`. A bare `sudo i2cdetect` without port select will not list chips behind the mux.
